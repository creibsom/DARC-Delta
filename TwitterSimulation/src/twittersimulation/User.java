...
public class User implements Steppable {
     ...
     * @param numTweetables Number of tweetables the user has
     * @param rn Random number generator for picking followees
     * @param exp Exponential distribution used to schedule tweeting
    */
    public User(int id, double freq, int numTweetables, MersenneTwisterFast rn, Exponential exp) {
        ...
        for(int i = 0; i < numTweetables; i++) {
           temp = Math.abs(this.rn.nextInt() % TwitterSimul.NUM_USERS);
